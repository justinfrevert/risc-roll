...
pub(crate) struct Arguments {
	...
	pub sig: String,
	#[clap(short, help = "Mini Secrate Key of sender for signing")]
	pub mini_sec: String,
}
...
async fn main() {
	...
	let sender_public_key = Public::from_h256(H256::from_str(&args.sender).unwrap());
	let receiver_public_key = Public::from_h256(H256::from_str(&args.receiver).unwrap());
	...
	let signature = Signature::from_h512(H512::from_str(&args.sig).unwrap());

	if !verify_batch(
		vec![&transfer_amount.to_be_bytes()],
		vec![&signature],
		vec![&sender_public_key.into()],
	) {
	...
	let api = OnlineClient::<PolkadotConfig>::new().await.unwrap();
	let restored_key = SubxtPair::from_string(&args.mini_sec, None).unwrap();
