...
use subxt::{
	...
	ext::{
		sp_core::{sr25519::{Pair as SubxtPair, Public, Signature, verify_batch}, Pair as SubxtPairT},
...
pub(crate) struct Arguments {
	#[clap(short, help="Hex string of public key of sender")]
	pub sender: String,
	#[clap(short, help="Hex string of public key of receiver")]
	pub receiver: String,
	#[clap(short, help="Transfer amount")]
	pub amount: u128,
	#[clap(short, help="Signature string generated by sender with amount as data")]
	pub sig: String,
}
...
async fn main() {
	...
	let sender_result = account_query(&api, Public::from_h256(H256::from_string(args.sender)).into());
	let receiver_result = account_query(&api, Public::from_h256(H256::from_string(args.receiver)).into());
	let transfer_amount = args.amount;
	let signature = Signature::from_h512(H512::from_string(args.sig));

	if !verify_batch(vec![&transfer_amount.to_be_bytes()], vec![&signature], vec![&sender.into()]) {
		panic!("Signature varification failed");
	}


    let sender_free_balance = sender_result.unwrap().unwrap().data.free;
    let receiver_free_balance = receiver_result.unwrap().unwrap().data.free;

    println!("sender balance: {:?} recipient balance {:?}", sender_free_balance, receiver_free_balance);
    let receipt = transfer(
        sender_free_balance,
        receiver_free_balance,
        transfer_amount
    );
    ...
    let signer = PairSigner::new(sender.into());
