...
pub(crate) struct Arguments {
	...
	pub amount: u128,
	#[clap(long, help = "Signature string generated by sender with amount as data")]
	pub sig: String,
	#[clap(short, help = "Hex of Secrate Key of sender for signing")]
...
async fn main() {
	...
	let sender_pair = SubxtPair::from_seed(&array_bytes::hex2array_unchecked(&args.sender_hex_seed));
	...
	let message = b"hello";
	let sig2  = sender_pair.sign(message);
	if !SubxtPair::verify(&sig2, &message, &sender_public_key) {
		panic!("Oh HollyCrap");
	}

	let signature = Signature::from_raw(array_bytes::hex2array_unchecked(&args.sig));
		println!("sig2 :{:?}, signature : {:?}", sig2, signature);
	if sig2 != signature {
		panic!("Oh Shiva");
	}
	if !SubxtPair::verify(&signature, &message, &sender_public_key) {
		panic!("Oh Crap");
	}
	// if !verify_batch(
	// 	vec![b"1000"],
